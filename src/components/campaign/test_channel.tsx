...
import EditChannelFlyot from "../channels/edit_channel_flyot";
...
const TestChannelDetails = () => {
  const router = useRouter();
  const { data: initialData } = useGetTemplates<Template>(router.query.id);
  const { data, isLoading } = useGetChannels<Channels>(initialData.channel.toString());
...
export default TestChannelDetails;
